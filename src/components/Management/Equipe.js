...
const Equipe = props => {
	...
	console.log(`[Equipe] Current Header is =${centerComponent.text}`);
	const clicked = () => centerComponent.text === "Fiche Equipe";

	console.log(`[Equipe] Click=${clicked()}`);
	...
	const isEquipe = () => (clicked() && page === "equipe" || !clicked() && page === "management" );

	const navToPages = () => (user.email !== undefined && !isEquipe());
		...
		navToPages()
			...
			: 		(
				<>
					{!clicked()
						? (
							<div className="equipe" id="equipe" onClick={() => onClickEquipe()}>
								<img src={PLUSPLUS} width="35px" height="35px" />
								Créer Fiche Equipe
							</div>
						)
						:					(
							<div className="equipeClicked" id="equipe">
